...
export function HeroSection() {
  ...
  const handleWhatsAppClick = () => {
    const message = encodeURIComponent(
      `Olá, meu nome é ${name} e gostaria de falar com o Dr. Sidnei Nardini sobre uma questão familiar. Pode me ajudar?`
    );
    ...
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center text-white max-w-4xl mx-auto px-4">
        ...
        <h1 className="text-4xl md:text-7xl font-bold mb-6 text-white animate-fade-in">
          Protegendo Seus Direitos e Patrimônio Familiar
        </h1>
        <p className="text-lg md:text-2xl mb-8 text-gray-200 px-4 animate-fade-in animate-delay-100">
          Assessoria jurídica humanizada para questões familiares e sucessórias, garantindo seus direitos com sensibilidade e eficiência
        </p>
        <Card className="bg-white/10 backdrop-blur-md border-white/20 p-6 animate-fade-in" style={{ animationDelay: "0.2s" }}>
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <span className="text-lg font-medium text-primary">Consulta sem compromisso</span>
          </div>
          <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
            <div className="w-full md:w-auto">
              ...
              <p className="text-xs text-white/60 mt-2">
                Experiência em resolver questões familiares com sensibilidade
              </p>
            </div>
            <Button
              onClick={handleWhatsAppClick}
              size="lg"
              className="bg-primary hover:bg-primary-dark text-white w-full md:w-auto hover-scale transition-all duration-300 shadow-lg hover:shadow-xl"
              disabled={!name.trim()}
            >
